...
export default function AdminPanel() {
  // --- ESTADOS DE AUTENTICAÇÃO E LAYOUT ORIGINAL ---
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [loginMode, setLoginMode] = useState('user'); // 'user' ou 'admin'
  const [usernameInput, setUsernameInput] = useState('');
  const [passwordInput, setPasswordInput] = useState('');
  const [adminTokenInput, setAdminTokenInput] = useState('');
  const [ownerId, setOwnerId] = useState(''); // CRUCIAL PARA CARREGAR DADOS
  ...
  useEffect(() => {
    ...
    if (savedOwner) {
        ...
        fetchData(savedOwner); // Carrega dados imediatamente
    }
  ...
  // --- LOGIN ADAPTADO PARA O SEU LAYOUT ---
  const handleLogin = async (e) => {
    ...
    if (loginMode === 'user') {
        try {
            const res = await fetch('/api/admin-login', { 
                ...
                body: JSON.stringify({ password: passwordInput }) // Usa a senha para validar
            });
    ...
    // Lógica para Admin (Se usar token ou senha mestra)
    else {
     ...
     try {
         ...
         while (true) {
             ...
             if (availableSenders.length === 0) {
                 addLog('⏳ Pausa geral (Flood Protection). Aguardando 1 min...');
             ...
             // SE ACABARAM LEADS: ESPERA (BOLA DE NEVE)
             if (leads.length === 0) {
                 addLog('❄️ Aguardando novos leads (30s)...');
